function amntBlur(element) {
		...
		if(amnt=="")
    	{
    		$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("尊敬的客户，请填写投保金额。");
        	return;
    	}
		if(/^[1-9]\d*00$/.test(amnt)){
			// 是不是大于19.99万
			if(parseInt(amnt) <= 5000000){
				amntTemp = amnt;
			}else{
				element.value = "";
				flag = false;
				$(".pop").show();
	        	$(".mask").show();
	        	$(".pop span").text("尊敬的客户，您的购买金额超过最高购买上限，请重新输入!");
			}
		}else{
			element.value = "";
			flag = false;
			$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("尊敬的客户，本产品的投保金额必须大于100,投保金额必须是100的整数倍。");
		}
		if(flag){
...
$(function () 
{
    ...
    $("#switch").on("click", function () {
        if ($(this).hasClass("turnOn")) {
            ...
            $('.bb').val($('.tb').val());
			$('.bb').data("occcode",$('#Tb_appntOccupation3').val());
			$('.bb').siblings('#occupationType').val($('#Tb_occupationType').val());
			$('.bb').css('pointer-events','none');
    ...
    // 输入框弹出键盘时改变样式
    /*
	 * $("input").on("focus", function () { if (!$(this).prop("readonly")) {
	 * $(".fixedBox").css("position", "static"); $(".header").css("position",
	 * "static"); $(".info").css("marginTop", "0");
	 * //$(".whetherRead").css("marginBottom", "1.375rem"); } }).on("blur",
	 * function () { $(".fixedBox").css({"position": "fixed", "bottom": "0"});
	...
	}else if($("#productNo").val()=="sel"){
		
		$("#birthday").on("change", function () {
    		// 年龄判断
    		if (getAgeByBirthDay($("#birthday").val()) < 18 || getAgeByBirthDay($("#birthday").val()) > 70) {
    			$("#birthday").parent().parent().siblings(".errorMsg").css("display", "inline-block").text("年龄必须在18~70岁之间！");
    		} else {
    			$(this).parent().parent().siblings(".errorMsg").css("display", "inline-block").text("").hide();
    		}
    		$(".insuredBirthdayReadonly").siblings(".mySelf").val($("#birthday").val());
	 ...
	 * $("#propertyArea").parent().siblings(".errorMsg").css("display",
	 * "inline-block").text("").hide(); });
	 * 
	 * $(".occ").on("click", function () {
	 * $(this).parent().siblings(".errorMsg").css("display",
	 * "inline-block").text("").hide(); }); $("#bankArea").on("click", function () {
	 * $(this).parent().siblings(".errorMsg").css("display",
	 * "inline-block").text("").hide(); });
	 * 
    ...
    // E家安康
    /*
	 * $(".EInsuredName").on("blur", function () { checkName($(this)); });
	 * $(".EInsuredIdNo").on("blur", function () {
	 * $(this).val($(this).val().toUpperCase()); checkID($(this)); });
	 */
    ...
    $("#insuredIdNo").on("blur", function () {
        ...
        if (getAgeByBirthDay($(".insuredBirthdayReadonly").val()) > 50 || getDayByBirthDay($(".insuredBirthdayReadonly").val()) < 28) {
    ...
    /** *************************续缴账户信息校验************************* */
        // 开户名校验
    /*
	 * $("#accountName").on("blur", function () { checkName($(this)); });
	 */
    // 银行卡号校验
  ...
  //w加的=========================================================================
    $("#insuredIdNo").on("blur", function () 
    {
        ...
        if ($("#insuredCardType").find("option:selected").text() == "身份证") {
            ...
            // 被保人年龄小于18岁的职业默认
            if (getAgeByBirthDay($("#insuredBirthday").val()) >= 0 && getAgeByBirthDay($("#insuredBirthday").val()) <=5) {
            	$('.bb').val("学龄前儿童");
    			$('.bb').data("occcode","2090115");
    			$('.bb').siblings('#occupationType').val("1");
    			$('.bb').css('pointer-events','none');
    			console.log($("#appntOccupation3")[0])
//    			console.log($("#appntOccupation3").val())
//    			console.log($("#occupationType").val())
    		} else if(getAgeByBirthDay($("#insuredBirthday").val()) > 5 && getAgeByBirthDay($("#insuredBirthday").val()) <= 17){
    			$('.bb').val("学生");
    			$('.bb').data("occcode","2090114");
    			$('.bb').siblings('#occupationType').val("1");
    			$('.bb').css('pointer-events','none');
    			console.log($("#appntOccupation3")[0])
//    			console.log($("#occupationType").val())
    			...
    			$('.bb').val("");
    			$('.bb').data("occcode","");
    			$('.bb').siblings('#occupationType').val("");
    		}
    ...
    $("#sureToSave").click(function() {
        ...
        sendRequest("../../life/abz/newcontract_save.do",getData(),function (data) {
        	console.log(data);
        	var obj = JSON.parse(data);
        	if(obj.code == 0){
        		...
        		console.log(orderNo);
        		// var returnPage = obj.data.returnPage;
        		window.location.href = "../../life/abz/contractpay.do?orderNo="+orderNo;
        		//window.location.href = "../../life/abz/toldinfo.do?orderNo="+orderNo+"&riskCode="+riskCode;
            }else{
            	$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text(obj.msg);
    ...
    // 进入页面清空开户行和开户所在第内容
    /*
	 * $("#bankArea").val(""); $("#accountBank_dummy").val("");
	 * $("#cardNo").val(""); $("#cardNoAgain").val("");
	 * $("#accountBank_dummy").attr("placeholder","请选择");
	 * $("#accountBank").find("option").each(function(){
	 * if($(this).text()=="请选择"){ $(this).prop("selected",true);
	 ...
	 * if($("#accountBank").find("option:selected").val() != "" &&
	 * $("#bankArea").siblings("input")[1].value != ""){ getSubBank(path
	 * +"/pay/getSubBankInfo",{
	 * bankCode:$("#accountBank").find("option:selected").val(),
	 * banksProvince:$("#bankArea").siblings("input")[1].value,
	 * banksCity:$("#bankArea").siblings("input")[2].value },function(data){ var
	 * obj = JSON.parse(data); $("#accountSubBank").html('<option value="">请选择</option>');
	 * var htm = ""; for(var i = 0;i < obj.data.length;i++){
	 * if(obj.data[i].bankNo == $("#subBankCode").val()){ htm += '<option
	 * selected value="'+obj.data[i].bankNo+'">'+obj.data[i].bankName+'</option>';
	 * }else{ htm += '<option
	 * value="'+obj.data[i].bankNo+'">'+obj.data[i].bankName+'</option>'; } }
	 * $("#accountSubBank").append(htm);
	 * $("#accountSubBank").scroller('destroy').scroller($.extend({preset:
	 * 'select'}, { theme: "android-ics light", lang: "zh", display: 'bottom',
	 * rtl: true, inputClass: 'tmp', onSelect: function () { if
	 * ($(this).find("option:selected").text()=="请选择"){
	 * $(this).siblings(".tmp").css("color","#999999"); }else{
	 * $(this).siblings(".tmp").css("color","#222222"); } } })); },function(){}) } }
	 */
...
// 投保人信息
    // 保额
    if ($("#mainPrice").val()==null || $("#mainPrice").val()=="") {
    	flag = false;
	}
    // 姓名校验
    ...
    if ($("#Tb_occupationType").val()=="" || $("#Tb_occupationType").val()==null ){
    	$("#Tb_occupationType").parent().siblings(".errorMsg").css("display", "inline-block").text("请选择职业！");
    	flag = false;
    }else {
        $("#Tb_occupationType").parent().siblings(".errorMsg").css("display", "inline-block").text("").hide();
    }
    // 职业校验
    if ($(".appntOccupation").length>0) {
        if ($(".appntOccupation")[0].tagName == "INPUT") {
    ...
    if ($("#switch").hasClass("turnOff")) {
        if ($("#relation").find("option:selected").text() != "本人") {
            ...
            if ($("#insuredBirthday").length>0) {
                ...
                //投被保人不一致时 被保人年龄大于18岁的必须和投保人一致
                if (getAgeByBirthDay($("#insuredBirthday1").val()) > 18) {
                	 $("#insuredBirthday").parent().parent().siblings(".errorMsg").css("display", "inline-block").text("投被保人不一致时 被保人年龄大于18岁的 必须和投保人一致！");
                     flag = false;
				} else {
                    $("#insuredBirthday").parent().parent().siblings(".errorMsg").css("display", "inline-block").text("").hide();
                }
            ...
            if ($("#occupationType").val()=="" || $("#occupationType").val()==null ){
            	$("#occupationType").parent().siblings(".errorMsg").css("display", "inline-block").text("请选择职业！");
            	flag = false;
            }else {
                $("#occupationType").parent().siblings(".errorMsg").css("display", "inline-block").text("").hide();
            }
...
//            alert($("#insuredArea").val())
    ...
    if($("#productNo").val() == "sel")
    {
    	...
    	if($("#amnt").val()<100)
    	{
    		$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("尊敬的客户，本产品的投保金额必须大于100，请调整投保金额。");
        	return;
    	}
    	...
    	if($("#amnt").val()%100>0)
    	{
    		$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("尊敬的客户，本产品的投保金额必须是100的整数倍，请调整投保金额。");
        	return;
    	}
    	
		if($("#mainPrice").val()<100)
		{
			$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("尊敬的客户，期交最低保费不得低于100元，请降低保额投保。");
        	return;
		}
		if($("#mainPrice").val()>5000000)
		{
			$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("尊敬的客户，期交最低保费不得高于5000000元，请调整保额投保。");
        	return;
		}
    }
...
//    	if($("#payendyear").find("option:selected").val()=="15Y" || $("#payendyear").find("option:selected").val()=="20Y")
//    	{
    		if(amntage>55)
    		{
    			$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text("尊敬的客户，此被保人年龄不符合产品投保要求。");
            	return;
    		}
//    	}
    	
    	//地区:  一类机构0-17周岁最高可投保30万；18-40岁最高可投保50万；41-50岁最高可投保30万； 51-55岁最高可投保10万。
    	//    	二类机构0-17周岁最高可投保30万；18-40岁最高可投保40万；41-50岁最高可投保20万；51-55岁最高可投保5万
    	//     （一类机构为北京、上海、广东、深圳、浙江、江苏 ；二类机构为除一类机构以外的其他机构
//    		alert("省份是：：：：："+$("#appntProvince").val())
    		if ($("#appntProvince").val()=="110000" || $("#appntProvince").val()=="310000" || $("#appntProvince").val()=="440000" 
    			|| $("#appntProvince").val()=="330000" || $("#appntProvince").val()=="320000") {
				
    			if(amntage>=0 && amntage<=17 && $("#amnt").val()>300000){
    				$(".pop").show();
    				$(".mask").show();
    				$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在0~17岁时，最高投保金额为30万元。");
    				return;
    			}
    			else if(amntage>=18 && amntage<=40 && $("#amnt").val()>500000){
    				$(".pop").show();
    				$(".mask").show();
    				$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在18~40岁时，最高投保金额为50万元。");
    				return;
    			}
    			else if(amntage>=41 && amntage<=50 && $("#amnt").val()>300000){
    				$(".pop").show();
    				$(".mask").show();
    				$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在41~50岁时，最高投保金额为30万元。");
    				return;
    			}
    			else if(amntage>=51 && amntage<=55 && $("#amnt").val()>100000){
    				$(".pop").show();
    				$(".mask").show();
    				$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在51~55岁时，最高投保金额为10万元。");
    				return;
    			}
			}else{
				if(amntage>=0 && amntage<=17 && $("#amnt").val()>300000){
    			...
    			else if(amntage>=18 && amntage<=40 && $("#amnt").val()>400000){
    				$(".pop").show();
    				$(".mask").show();
    				$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在18~40岁时，最高投保金额为40万元。");
    				return;
    			}
    			else if(amntage>=41 && amntage<=50 && $("#amnt").val()>200000){
    				$(".pop").show();
    				$(".mask").show();
    				$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在41~50岁时，最高投保金额为20万元。");
    				return;
    			}
    			else if(amntage>=51 && amntage<=55 && $("#amnt").val()>50000){
    				$(".pop").show();
    				$(".mask").show();
    				$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在51~55岁时，最高投保金额为5万元。");
    				return;
    			}
			}
	}
...
function getData(){
	...
	/*
	 * var appFactorInfo=[]; $(".iFactor").each(function(index, el){ var obj =
	 * {}; if(el.tagName == "SELECT"){ obj.apptype = $(this).data("type");
	 * obj.appFactorCode = $(this).data("appfactorid"); obj.isPremCalFacotor =
	 * $(this).data("ispremcal"); obj.appValue =
	 * $(this).find("option:selected").val(); obj.appShowValue =
	 * $(this).find("option:selected").text(); obj.conjuncDutyCode =
	 * $(this).data("conjuncdutycode"); }else if(el.tagName == "INPUT" ||
	 * el.tagName == "TEXTAREA"){ obj.apptype = $(this).data("type");
	 * obj.appFactorCode = $(this).data("appfactorid"); obj.isPremCalFacotor =
	 * $(this).data("ispremcal"); obj.appValue = $(this).data("value");
	 * obj.appShowValue = $(this).val(); obj.conjuncDutyCode =
	 * $(this).data("conjuncdutycode"); } appFactorInfo.push(obj); }); var
	 * dutyInfo = []; $(".insurInfo").each(function(index, el){ var obj = {};
	 * if(el.tagName == "SELECT"){ obj.dutyNo = $(this).data("code"); obj.amnt =
	 * $(this).find("option:selected").val(); obj.showAmnt =
	 * $(this).find("option:selected").text(); obj.prem = $(this).data("prem");
	 * obj.isPremCalFacotor = $(this).data("ispremcal"); }else if(el.tagName ==
	 * "INPUT" || el.tagName == "TEXTAREA"){ obj.dutyNo = $(this).data("code");
	 * obj.amnt = $(this).data("value"); obj.showAmnt = $(this).val(); obj.prem =
	 * $(this).data("prem"); obj.isPremCalFacotor = $(this).data("ispremcal"); }
	 * dutyInfo.push(obj); }); var data = {
	...
	var data = 
	{
		...
		//"insuredAge":getAgeByBirthDay($("#insuredBirthday1").val()),
...
// };
    ...
    if ($("#insuredBirthday1").val()!="" && $("#insuredBirthday1").val()!=null) {
    	oObj["insuredAge"] = getAgeByBirthDay($("#insuredBirthday1").val());
	}
...
function calPrice()
{
	 ...
	 * 各种校验，校验通过了，才进行保费计算
	 */
	if ("cqs"==$("#productNo").val()) {
		if($("#amnt").find("option:selected").val()==null
				||$("#amnt").find("option:selected").val()=="")
		{
			return;
		}
		...
		if($("#relation").find("option:selected").val()==null
			...
			if($("#relation").find("option:selected").val()=="00")
			{
				...
				if ($("#insuredBirthday1").val()==null || $("#insuredBirthday1").val()=="") 
				{
					return ;
				}
	 ...
	 * ||$("#ishuanmian").find("option:selected").val()=="") { return; }
	 */
	sendPriceRequest("../../life/abz/getprice02.do",getData(),function (data) 
	{
    	var obj = JSON.parse(data);
    	console.log(obj);
    	if(obj.code == 0)
    	{
    		...
    		$("#insurePrice").html(obj.insurePrice);
    	}
    	else
    	{
    		$(".pop").show();
	...
	/*
	 * var getPriceUrl = path +"/prem/getProductPrem"; if($("#productNo").val() ==
	 * 410101020){ getPriceUrl = path +"/prem/getGroupProductPrem"; }
	 * sendPriceRequest(getPriceUrl,getData(),function (data) { var obj =
	 * JSON.parse(data); console.log(obj); if(obj.code == 0){
	 * $("#insurePrice").val(obj.data.totlePrem); if($("#productNo").val() ==
	 * 410101020){ for(var i in obj.data.insuredList){
	 * $("."+i).find(".groupPrice").val(obj.data.insuredList[i]); } } //
	 * if(obj.data.dutyFactorPremList){ // for(var i =0;i <
	 * obj.data.dutyFactorPremList.length;i++){ //
	 * $(".infoList").find(".change_"+obj.data.dutyFactorPremList[i].dutyCode).val(obj.data.dutyFactorPremList[i].amnt) // } // }
	 * }else{ if(obj.data.totlePrem != ""){ $(".pop").show(); $(".mask").show();
	 * $(".pop span").text(obj.msg); } } },function(){});
	 */
}
// 选择投保人身份证时自动获取生日和性别
