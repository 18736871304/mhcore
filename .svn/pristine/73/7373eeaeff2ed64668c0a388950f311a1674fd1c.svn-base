		...
		<head>
			...
			<script>
				...
				window.onload = function () {
					...
					$('#q02org').combobox('setValue', '100200000');
					organAfterSelect($('#q02org'))
					$('#q03org').combobox('setValue', '100200100');
					organAfterSelect($('#q03org'))

					$('#q04org').combobox('setValue', '100200101');
					organAfterSelect($('#q04org'))
				...
				function fypquery(orderbystr, insType) {
					...
					tParam.usergrade = $('#qusergrade').combobox('getValues').join(",");
				...
				function displaysumdata(data) {
					if (data == null || data == "") {
						...
						$('#ylpolicycount').val("");
						$('#ylprem').val("");
						$('#company_sumfyp20').val("");
						$('#shanxi_changxian_sumfyp20').val("");
						...
						$('#company_sumfyp20').val(data.company_sumfyp20);
						$('#shanxi_changxian_sumfyp20').val(data.shanxi_changxian_sumfyp20);
				...
				function biaobao(val, row, index) {
					console.log(row)
					if (row.organcode === '100200101') {
						return row.shanxi_changxian_sumfyp20
					} else {
						return row.changxian_sumfyp20;
					}
				}



				function zhesuanBiaobao(val, row, index) {
					console.log(row)
					if (row.organcode === '100200101') {
						return row.shanxi_coefficient_changxian_sumfyp20
					} else {
						return row.coefficient_changxian_sumfyp20;
					}
				}
			...
			<div style="margin-left:0%">
				<table class="common">
					<tr>
						...
						<td class="report_common">
							<select class="easyui-combobox" panelHeight="auto" style="width:160%" name="qworkyears"
								id="qworkyears" data-options="multiple:true">
							</select>
						</td>
					</tr>
				...
				<table id="policyList" class="easyui-datagrid" title="个人排名" style="width:auto;height:auto"
					...
					<thead>
						<tr>
							...
							<th data-options="field:'changxian_sumfyp20',width:80,formatter:biaobao">长期险标保</th>
							<th
								data-options="field:'coefficient_changxian_sumfyp20',width:130, formatter:zhesuanBiaobao">
								长期险折算系数后标保</th>

							<th
								data-options="field:'coefficientSJsumfyp20',width:90,formatter:function(value){return value=='0.00' ? '' : value;}">
								其中升级部分</th>
							<th
								data-options="field:'coefficientBSJsumfyp20',width:120,formatter:function(value){return value=='0.00' ? '' : value;}">
								其中被升级部分</th>
				...
				<table class="common" style="width: 100%;">
					<tr>
						...
						<td class="report_common">
							<input class="txt" id="shanxi_changxian_sumfyp20" name="shanxi_changxian_sumfyp20" readonly>
						</td>

						<td class="reprot_title" style="width: 9%;">
							公司被升级部分合计
						</td>
						<td class="report_common">
							<input class="txt" id="company_sumfyp20" name="company_sumfyp20" readonly>
						</td>
				...
				<br>
				<font color="red">其中升级部分 - 03和04个人部分折算系数后标保  	<br>
					其中被升级部分 - 03其他员工部分折算系数后标保</font>
			</div>
		</body>
