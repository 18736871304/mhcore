$(function () 
{
    ...
    */
    ...
    $("#insuredIdNo").on("blur", function () {
        ...
        if (getAgeByBirthDay($(".insuredBirthdayReadonly").val()) > 50 || getDayByBirthDay($(".insuredBirthdayReadonly").val()) < 30) {
            $("#insuredIdNo").parent().parent().siblings(".errorMsg").css("display", "inline-block").text("被保险人超投保年龄范围");
        } else {
    ...
    $("#sureToSave").click(function() {
    	...
    	*/
    	...
    	//alert(123);
    	if($("#insurePrice").html()==""||$("#sumPrice").val()=="0")
    	{
    		$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("普通款，年龄+缴费年期不能大于60；PLUS款，年龄+缴费年期不能大于65");
        	return;
    	}
    	
        sendRequest("../../life/sunlife/newcontract_save.do",getData(),function (data) {
        	console.log(data);
        	var obj = JSON.parse(data);
        	if(obj.code == 0){
        		...
        		console.log(orderNo);
        		//var returnPage = obj.data.returnPage;
        		window.location.href = "../../life/sunlife/contractpay.do?orderNo="+orderNo;
            }else{
            	$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text(obj.msg);
...
//投保人信息
    ...
    */
    ...
    if($("#productNo").val() == "risk01"||$("#productNo").val() == "risk02")
    {
    	if($("#amnt").val()<50000)
    	{
    		$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("尊敬的客户，本产品的投保金额必须大于50000，请调整投保金额。");
        	return;
    	}
    	...
    	if($("#amnt").val()%1000>0)
    	{
    		$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("尊敬的客户，本产品的投保金额必须是1000的整数倍，请调整投保金额。");
        	return;
    	}
    	...
    	//console.log($('#insuredBirthday').val());
    	//var amntage = getAgeByBirthDay($('#insuredBirthday').val());
    	
    	if($("#productNo").val() == "risk01")
    	{
    		if(amntage>=0 && amntage<=17 && $("#amnt").val()>600000){
    			$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在0~17岁时，最高投保金额为60万元。");
            	return;
    		}
        	...
        	else if(amntage>=41 && amntage<=45 && $("#amnt").val()>300000){
    			$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在41~45岁时，最高投保金额为30万元。");
            	return;
    		}
        	else if(amntage>=46 && amntage<=50 && $("#amnt").val()>200000){
    			$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在46~50岁时，最高投保金额为20万元。");
            	return;
    		}
    	}
    	
    	if($("#productNo").val() == "risk02")
    	{
    		if(amntage>=0 && amntage<=9 && $("#amnt").val()>200000){
    			$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在0~9岁时，最高投保金额为20万元。");
            	return;
    		}
    		else if(amntage>=10 && amntage<=17 && $("#amnt").val()>450000){
    			$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在10~17岁时，最高投保金额为45万元。");
            	return;
    		}
        	else if(amntage>=18 && amntage<=40 && $("#amnt").val()>300000){
    			$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在18~40岁时，最高投保金额为30万元。");
            	return;
    		}
        	else if(amntage>=41 && amntage<=45 && $("#amnt").val()>220000){
    			$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在41~45岁时，最高投保金额为22万元。");
            	return;
    		}
        	else if(amntage>=46 && amntage<=50 && $("#amnt").val()>150000){
    			$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text("尊敬的客户，本产品的被保险人，年龄在46~50岁时，最高投保金额为15万元。");
            	return;
    		}
    	}
    	...
    	if($("#payendyear").find("option:selected").val()=="0Y")
    	{
    		if($("#mainPrice").val()<5000)
    		{
    			$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text("尊敬的客户，趸交最低保费不得低于5000元，请降低保额投保。");
            	return;
    		}
    	}
    	else
...
function getData(){
    ...
    */
	...
	var data = 
	{
		"insureplan":$("#insureplan").find("option:selected").val(),
		...
		"appnt_height":$("#appnt_height").val(),
		"appnt_weight":$("#appnt_weight").val()
	};
...
function calPrice()
{
	...
	if($("#insureplan").find("option:selected").val()==null
	 ||$("#insureplan").find("option:selected").val()=="")
	{
		return;
	}
	...
	sendPriceRequest("../../life/sunlife/premiumtrial.do",getData(),function (data) 
	{
    	var obj = JSON.parse(data);
    	console.log(obj);
    	if(obj.code == 0)
    	{
    		...
    		$("#hmPrice").val(obj.hmprem);
    		$("#qzPrice").val(obj.qzprem);
    		$("#sumPrice").val(obj.sumprem);
