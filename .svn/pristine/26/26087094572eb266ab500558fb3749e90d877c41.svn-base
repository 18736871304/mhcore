...
var insuranceColor = [
                  	{name: '意外险',color: '#444242'},
                  	{name: '医疗险',color: '#1b1b1b'},
                  	{name: '重疾险',color: '#e02828'},
                  	{name: '年金险',color: '#d7d7d7'},
                  	{name: '人寿险',color: '#a1a1a1'},
                  	{name: '理财险',color: '#dc220d'}
                  ]
...
var indexFunc = {
	...
	bindEvent: function () 
	{
		...
		$('body').on('click', '.list-box', function () 
		{
			...
			window.location.href = '/life/common/policyDetail.do?orderid='+policyid+'&insuredid='+insureid
		})
...
function listTpl(data) {
	...
	data.forEach(function (item) {
		...
		tpl += '<div class="list-box '+item.status+'" data-insureid="'+item.insuredserialno+'" data-policyid="'+item.orderid+'">' +
			'    <div class="list-wrap">' +
