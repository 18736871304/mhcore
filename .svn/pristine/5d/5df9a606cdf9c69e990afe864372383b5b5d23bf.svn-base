...
function chooseprotocol()
{
	...
	if(row==null)
	{
		$.messager.alert('操作提示','请选中协议的数据！','error');
		return;
	}
	
	if($('#startdate').datebox("getValue")==null||$('#startdate').datebox("getValue")=="")
	{
		$.messager.alert('操作提示','计算起期不能为空！','error');
		return;
	}
	
	if($('#enddate').datebox("getValue")==null||$('#enddate').datebox("getValue")=="")
	{
		$.messager.alert('操作提示','计算止期不能为空！','error');
		return;
	}
	
	var tturl = "lifefinance/getPolicyist.do";
	...
	//用登录机构查询
	//tParam.organcode = row.organcode;
	//alert(tParam.organcode)
	//tParam.policyState = "02";
	
	displayDataGrid($('#policylist'),tParam,tturl);
}
...
function balancecal()
{
	...
	/*
	alert(tparam.lifeprotocolserialno);
	alert(tparam.startdate);
	alert(tparam.enddate);
	alert(tparam.insorgancode);
	alert(tparam.organcode);
	*/
	
	ajaxdeal("lifefinance/balanceCal.do",tparam,null,null,saveSuss);
}
...
<div style="margin-left:0%">
	...
	<table class = "common">
		<tr>
			...
			<td class = "common">
				<input class="easyui-datebox" style="width: 160%" id="startdate" name="startdate">
			</td>
			<td class = "title">
				计算止期
			</td>
			<td class = "common">
				<input class="easyui-datebox" style="width: 160%" id="enddate" name="enddate">
			</td>
			<td>
			</td>
			<td>
	...
	<br>
	<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-edit'" onclick = "chooseprotocol()">选择协议</a>
 	<!-- 
 	<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-edit'" id = "protocolpolicydownload" onclick = "protocolpolicydownload()">协议保单下载</a>
 	 -->
	<br>
	<br>
	...
	<br>
	<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-edit'" id = "balancecal" onclick = "balancecal()">协议计算</a>
