...
<body>
	<div class="mainIndex_wrapper">
		<div class="main_search">
			...
			<input class="serchInput" type="text" placeholder="搜索产品"/> <a
				href="javascript:;" class="serchBtn">搜索</a>
		</div>
		<div class="main_nav">
	...
	<script>
		...
		search.prototype = {
			...
			focusRander:function(){
				...
				if(this.Shistory.length>3){
					for(var i=0;i<3;i++){
						$('<li class="item"></li>').html('<img src="./images/life/search-item-icon.png" /><div class="txt hisitem">' + this.Shistory[i]+ '</div> <a class="clearItem">×</a>').appendTo('.history-wrap .list')
					}
					$('.showAllBtn').css('display','block');
				}else{
					this.Shistory.forEach(function(item, index) {
						$('<li class="item"></li>').html('<img src="./images/life/search-item-icon.png" /><div class="txt hisitem">' + item+ '</div> <a class="clearItem">×</a>').appendTo('.history-wrap .list')
					})
					if(this.Shistory.length>0){
						$('.clearAllBtn').css('display','block');
					}
			...
			update : function() {
				...
				$('.main_search').on('click', '.serchBtn', function() {
					...
					that.Shistory.unshift(val);
					Lstorage.setItem('Shistory', that.handleArr(that.Shistory));
					that.render();
					
					window.location.href = "./searchResult.do?keywords="+val;
				})
			},
			deleteItem : function() {
				var that=this;
		...
		function disList(risklist, obj) 
		{
			for (var i = 0; i < obj.homeList.length; i++) 
			{
				...
				risklist.append('<div class="contentDescone" id="lclist">'
