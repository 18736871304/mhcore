...
//    	}
    	...
    	if($("#riskcode").val()=="110025")
    	{
    		...
    		if ($("#sumPrem").val() * paycount >=200000) 
    		{
    			$(".pop").show();
    	       	$(".mask").show();
    	       	$(".pop span").text("保费超过20万，暂不支持投保！");
    	       	return;
    		}
    	}
    	*/
    	
    	if($("#riskcode").val()=='110084')
    	{
    		var jy_age = 0;
        	
        	if($("#relation").val()!="00") 
            {
        		jy_age = getAgeByBirthDay($("#insuredBirthday").val());
        	}
            else
            {
            	jy_age = getAgeByBirthDay($("#birthday").val());
            }
        	
        	if(jy_age>=19&&$("#isshaoer").val()=='Y')
        	{
        		$(".pop").show();
    	       	$(".mask").show();
    	       	$(".pop span").text("18岁以上不能选择【少儿可选责任】!");
    	       	return;
        	}
    	}
    	...
    	/*
    	 * 复星特殊校验
    	 * 校验投保人年龄大于50周岁不能附加豁免险
    	 */
    	var tb_age = 0;
    	
    	tb_age = getAgeByBirthDay($("#birthday").val());
    	
    	if(tb_age>50&&$("#ishuomian").val()=="Y")
    	{
    		$(".pop").show();
	       	$(".mask").show();
	       	$(".pop span").text("投保人年龄大于50周岁不能附加豁免险！");
	       	return;
    	}
    	...
    	var payend_age = Number(jy_age) + Number($("#payendyear").val().replace("Y",""));
    	
    	if(payend_age>70)
    	{
    		$(".pop").show();
	       	$(".mask").show();
	       	$(".pop span").text("缴费期间+投保年龄不能大于70岁！");
	       	return;
    	}
    	*/
        ...
        sendRequest("../../life/fxlhjk/newcontract_save.do",getData(),function (data) {
        	console.log(data);
        	var obj = JSON.parse(data);
        	if(obj.code == 0){
...
//        		console.log(orderNo);
        		//var returnPage = obj.data.returnPage;
        		window.location.href = "../../life/fxlhjk/contractpay.do?orderNo="+orderNo;
            }else{
            	$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text(obj.msg);
...
{    
	var data = 
	{
		...
		"iszl":$("#iszl").find("option:selected").val(),
		"zlamnt":$("#zlamnt").find("option:selected").val(),
		"issecond":$("#issecond").find("option:selected").val(),
		"productplan":$("#productplan").find("option:selected").val(),
		"isshaoer":$("#isshaoer").find("option:selected").val(),
		"hassocialinsurance":$("#hassocialinsurance").find("option:selected").val()
	};
...
function calPrice()
{
	 ...
	 * 各种校验，校验通过了，才进行保费计算
	 */
	if($("#riskcode").val()=="110049"||$("#riskcode").val()=="110068"||$("#riskcode").val()=="110084")
	{
		if($("#relation").val()!="00")
		{
	...
	else
	{
		...
		if($("#insuyear").val()==null||$("#insuyear").val()=="")
		{
			return;
		}
				
		if($("#payendyear").val()==null||$("#payendyear").val()=="")
		{
			return;
		}
		
		if($("#ishuomian").val()==null||$("#ishuomian").val()=="")
		{
			return;
		}
	...
	sendPriceRequest("../../life/fxlhjk/calprem.do",getData(),function (data) 
	{
    	var obj = JSON.parse(data);
    	console.log(obj);
    	...
    	if(obj.code == 0)
    	{
    		...
    		$("#zlPrice").val(Number(obj.zlPrem).toFixed(2));
    		
    		$("#ysduty1").val(Number(obj.ysduty1).toFixed(2));
    		$("#ysduty2").val(Number(obj.ysduty2).toFixed(2));
    		
    		$("#duty01price").val(Number(obj.ysduty1).toFixed(2));
    		$("#duty02price").val(Number(obj.ysduty2).toFixed(2));
    		
    		$("#duty03price").val(Number(obj.ysduty3).toFixed(2));
    		$("#duty04price").val(Number(obj.ysduty4).toFixed(2));
    		
    		$("#duty05price").val(Number(obj.ysduty5).toFixed(2));
    	}
    	else
    	{
    		$(".pop").show();
...
function distwolevel()
{
	sendRequest("../../life/fxlhjk/get_twolevel.do",
			...
			function (data) 
	{
	   	...
	   	if(obj.code == 0)
	   	{
	   		$("#twolevel").html('')
	   		$("#twolevel").val('');
	   		$("#twolevel_dummy").val($("#twolevel").find("option:selected").text());
	   		
	   		$("#twolevel").append("<option value='0'>请选择</option>");
	   		for(var i=0;i<obj.twoList.length;i++)
	   		{
	   			$("#twolevel").append("<option value='"+obj.twoList[i].twolevelcode+"'>"+obj.twoList[i].twolevelname+"</option>");
	   		}
...
function disthreelevel()
{
	$("#twolevel_dummy").val($("#twolevel").find("option:selected").text());
	
	sendRequest("../../life/fxlhjk/get_threelevel.do",
			{"twolevel":$("#twolevel").find("option:selected").val()},
			function (data) 
	{
	   	console.log(data);
	   	var obj = JSON.parse(data);
	   	if(obj.code == 0)
	   	{
	   		...
	   		{
	   			$("#occupation").append("<option value='"+obj.threeList[i].jobcode+"'>"+obj.threeList[i].jobname+"</option>");
	   		}
