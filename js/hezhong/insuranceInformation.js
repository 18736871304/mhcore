...
//                        console.log($(".appntOccupation").find("option:selected").text())
    ...
    $("#idDate").on("change", function () {
        if($("#checkbox").hasClass("unCheckBox")){
            if(!getPersonOutNowDay($("#idDate").val())){
                $("#idDate").parent().parent().siblings(".errorMsg").css("display", "inline-block").text("有效期不能大于20年或小于0！");
            }
        }
        $(this).parent().parent().siblings(".errorMsg").css("display", "inline-block").text("").hide();
    ...
    */
    ...
    $("#insuredIdNo").on("blur", function () {
        ...
        if (getAgeByBirthDay($(".insuredBirthdayReadonly").val()) > 17 || getDayByBirthDay($(".insuredBirthdayReadonly").val()) < 0) {
...
//    		console.log($(this).data("value"))
    ...
    $("#sureToSave").click(function() {
        ...
        sendRequest("../../life/hz/newcontract_save.do",getData(),function (data) {
        	console.log(data);
        	var obj = JSON.parse(data);
        	if(obj.code == 0){
...
//        		console.log(orderNo);
        		//var returnPage = obj.data.returnPage;
        		// window.location.href = "../../life/hz/contractpay.do?orderNo="+orderNo+"&riskcode="+riskcode;
                window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx37a5a6ceced4f359&redirect_uri=http://insure.meihualife.com/life/hz/contractpay.do&response_type=code&scope=snsapi_base&state="+orderNo+"#wechat_redirect"
...
function calPrice()
{
	 ...
	 * 各种校验，校验通过了，才进行保费计算
	 */
	if($("#productNo").val() == "634"){
		
		console.log($("#prem").val());
	...
	sendPriceRequest("../../life/hz/getprice02.do",getData(),function (data)
	{
    	var obj = JSON.parse(data);
    	console.log(obj);
    	if(obj.code == 0)
    	{
    		if ($("#productNo").val()=="634")
    		{
    			$("#mainPrice").val(Number(obj.mainPrice).toFixed(2));
        		$("#insurePrice").html(Number(obj.insurePrice).toFixed(2));
                $("#hmPrice").val(Number(obj.hmPrice).toFixed(2));
        		$("#prem").val(Number(obj.prem).toFixed(2));
			}else if($("#productNo").val()=="636")
			{
                $("#insurePrice").html(Number(obj.insurePrice).toFixed(2));
                $("#prem").val(Number(obj.prem).toFixed(2));
            ...
            }else if($("#productNo").val()=="256")
			{
                $("#insurePrice").html(Number(obj.insurePrice).toFixed(2));
                $("#prem").val(Number(obj.prem).toFixed(2));
                if(obj.amnt==""||obj.amnt==null){
                    $("#amnt").val("不符合规则");
                }else{
                    $("#amnt").val(Number(obj.amnt).toFixed(2));
                }
            }
			else
			{
				...
				$("#insurePrice").val(Number(obj.insurePrice).toFixed(2));
...
function getArea(id){
    ...
    $("#"+id).on("click", function () {
        if($("#productNo").val()=="636"){
            var oneLevelId = "";
            var twoLevelId = "";
            var threeLevelId = "";
            var $this=$(this);
            oneLevelId = $("#newCar_price_lable").attr('data-province-code');
            twoLevelId = $("#newCar_price_lable").attr('data-city-code');
            threeLevelId = $("#newCar_price_lable").attr('data-district-code');
            ...
            var threeLevelId = "";
            var $this=$(this);
            oneLevelId = $("#insuredProvince").attr('data-province-code');
            twoLevelId = $("#insuredCity").attr('data-city-code');
            threeLevelId = $("#insuredCounty").attr('data-district-code');
            if ($this.siblings("input")[0].value == 2){
                var iosSelect = new IosSelect(2,
                    [iosProvinces, that.citysObject.iosCitys],
                    {
                ...
                var iosSelect = new IosSelect(3,
                    [iosProvinces, that.citysObject.iosCitys, that.countysObject.iosCountys],
                    {
                        ...
                        callback: function (selectOneObj, selectTwoObj, selectThreeObj) {
                            //先配置选择好的省市区Id,下次打开的时候默认选中之前选中好的省市区
                            //$("#insuredProvince").attr('data-province-code', selectOneObj.id);
                            //$("#insuredCity").attr('data-city-code', selectTwoObj.id);
                            //$("#insuredCounty").attr('data-district-code', selectThreeObj.id);
                            //把 “请选择行驶区域” 改为选好的省市区
                            $("#"+id).val(selectOneObj.value + ' ' + selectTwoObj.value + ' ' + selectThreeObj.value);
                            $this.siblings("input")[1].value = selectOneObj.codevalue;
                            $this.siblings("input")[2].value = selectTwoObj.codevalue;
...
{
    sendRequest("../../life/hz/get_twolevel.do",{"onelevel":$("#"+onelevel+"").find("option:selected").val()},function (data)
    {
        console.log(data);
        var obj = JSON.parse(data);
        if(obj.code == 0)
...
function disthreelevel(twolevel,threelevel)
{
    ...
    sendRequest("../../life/hz/get_threelevel.do",{"twolevel":$("#"+twolevel+"").find("option:selected").val()},function (data)
    {
        console.log(data);
        var obj = JSON.parse(data);
        if(obj.code == 0)
