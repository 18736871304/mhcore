...
//                        console.log($(".appntOccupation").find("option:selected").text())
    ...
    $("#birthday").on("change", function () {
        //年龄判断
        if (getAgeByBirthDayjia1($("#birthday").val()) < 18) {
    ...
    */
    ...
    $("#idNo").on("blur", function () 
    {
        ...
        if ($("#cardType").find("option:selected").text() == "身份证") {
            ...
            $(".insuredSexReadonly").siblings(".mySelf").val($(".sexReadonly").val());
            //年龄判断
            if ($(".birthdayReadonly").val()== null || $(".birthdayReadonly").val() == "" || getAgeByBirthDayjia1($(".birthdayReadonly").val()) < 18) {
    ...
    {
    	if(checkEmpty($("#insBeginDate").val())){
        	$("#insBeginDate").parent().parent().siblings(".errorMsg").css("display", "inline-block").text("请选择起保日期！");
        	flag = false;
        }else {
        	$("#insBeginDate").parent().parent().siblings(".errorMsg").css("display", "inline-block").text("").hide();
        }
    });
    ...
    $("#salary").on("blur", function () 
    {
    	if(checkEmpty($("#salary").val())){
        	$("#salary").parent().siblings(".errorMsg").css("display", "inline-block").text("请填写年收入！");
        	flag = false;
        }else {
        	$("#salary").parent().siblings(".errorMsg").css("display", "inline-block").text("").hide();
        }
    });
    ...
    */
    ...
    $("#insuredIdNo").on("blur", function () {
        ...
        if (getAgeByBirthDayjia1($(".insuredBirthdayReadonly").val()) > 60 || getDayByBirthDay($(".insuredBirthdayReadonly").val()) < 28) {
...
//    		console.log($(this).data("value"))
    ...
    $("#sureToSave").click(function() {
        ...
        sendRequest("../../life/anxin/newcontract_save.do",getData(),function (data) {
        	console.log(data);
        	var obj = JSON.parse(data);
        	if(obj.code == 0){
...
//        		console.log(orderNo);
        		//var returnPage = obj.data.returnPage;
        		window.location.href = "../../life/anxin/contractpay.do?orderNo="+orderNo+"&riskcode="+riskcode;
            }else{
            	$(".pop").show();
            	$(".mask").show();
            	$(".pop span").text(obj.msg);
	...
	{
		sendRequest("../../life/anxin/get_twolevel.do",{"onelevel":$("#"+onelevel+"").find("option:selected").val()},function (data) 
		{
		   	console.log(data);
		   	var obj = JSON.parse(data);
		   	if(obj.code == 0)
		   	{
		   		...
		   		$("#"+twolevel+"").val(obj.twoList[0].twolevelcode);
    ...
    function disthreelevel(twolevel,threelevel)
    {
    	...
    	sendRequest("../../life/anxin/get_threelevel.do",{"twolevel":$("#"+twolevel+"").find("option:selected").val()},function (data) 
    	{
    	   	console.log(data);
    	   	var obj = JSON.parse(data);
    	   	if(obj.code == 0)
...
//投保人信息
    ...
    if ($("#idNo").length>0) {
        ...
        } else if (getAgeByBirthDayjia1($(".birthdayReadonly").val()) < 18) {
            //年龄判断
            $("#idNo").parent().siblings(".errorMsg").css("display", "inline-block").text("年龄必须满18周岁！");
            flag = false;
        } else {
    ...
    if ($("#birthday").length>0) {
        if (!($("#cardType").find("option:selected").text() == "身份证")) {
            if ($("#birthday").val() == "") {
                ...
                flag = false;
            } else if (getAgeByBirthDayjia1($("#birthday").val()) < 18) {
                //年龄判断
                $("#idNo").parent().siblings(".errorMsg").css("display", "inline-block").text("年龄必须满18周岁！");
                flag = false;
            } else {
    ...
    //起保起期校验
    if ($("#insBeginDate").length>0) {
    	console.log("insBeginDate")
        if(checkEmpty($("#insBeginDate").val())){
        	console.log($("#insBeginDate").val()+"111")
        	$("#insBeginDate").parent().parent().siblings(".errorMsg").css("display", "inline-block").text("请选择起保日期！");
        	flag = false;
        }else {
        	$("#insBeginDate").parent().parent().siblings(".errorMsg").css("display", "inline-block").text("").hide();
...
//        }
    ...
    var amntage = getAgeByBirthDayjia1(birth);
    var amntageDays=getDayByBirthDay(birth);
    
    if($("#aliasname").val() == "bwylPlus")
    {
    	if(amntage>45 || amntage<6){
    		$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("尊敬的客户，被保险人投保年龄不能小于6岁或者大于45周岁。");
        	return;
    	}
    	
    }else if($("#aliasname").val() == "axys"){
    	
    	if(amntage>70 || amntageDays<28){
    		$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("尊敬的客户，被保险人投保年龄不能小于28天或者大于70周岁。");
        	return;
    	}
    	
    }else if($("#aliasname").val() == "lnrywx"){
    	
    	if(amntage>80 || amntage<50){
    		$(".pop").show();
        	$(".mask").show();
        	$(".pop span").text("尊敬的客户，被保险人投保年龄不能小于50周岁或者大于80周岁。");
        	return;
    	}
    ...
    if($("#productNo").val() == 110101017){
    	...
    	var age = getAgeByBirthDayjia1(birth);
...
function getData(){
    ...
    */
	...
	var data = 
	{
		...
		"amnt":$("#amnt").val(),
		"prem":$("#prem").val(),
		"age":getAgeByBirthDayjia1($("#birthday").val()),
...
//		"ishuanmian":$("#ishuanmian").find("option:selected").val()
    ...
    if ($("#insuredBirthday1").val()!="" && $("#insuredBirthday1").val()!=null) {
    	oObj["insuredAge"] = getAgeByBirthDayjia1($("#insuredBirthday1").val());
	}
...
function getAmntDesc(){
	...
	if(amnt== "56000"){
		...
		$("#ywyl_amnt").val("5000元");
		$("#jhc_amnt").val("1000元");
		$("#insurePrice").val("100.00");
		$("#disprice").html("100.00");
		$('#insurePrice').html("100.00");
	}else if(amnt== "111000"){
		$("#bz_amnt").val("10万元");
   		$("#sgsc_amnt").val("10万元");
		$("#ywyl_amnt").val("1万元");
		$("#jhc_amnt").val("1000元");
		$("#insurePrice").val("200.00");
		$("#disprice").html("200.00");
		$('#insurePrice').html("200.00");
	}else if(amnt== "222000"){
		$("#bz_amnt").val("20万元");
   		$("#sgsc_amnt").val("20万元");
		$("#ywyl_amnt").val("2万元");
		$("#jhc_amnt").val("2000元");
		$("#insurePrice").val("300.00");
		$("#disprice").html("300.00");
		$('#insurePrice').html("300.00");
	}else{
		$("#bz_amnt").val("30万元");
   		$("#sgsc_amnt").val("30万元");
		$("#ywyl_amnt").val("3万元");
		$("#jhc_amnt").val("3000元");
		$("#insurePrice").val("400.00");
		$("#disprice").html("400.00");
		$('#insurePrice').html("400.00");
	}
}
//计算保费
function calPrice()
{
	...
	sendPriceRequest("../../life/anxin/getprice02.do",getData(),function (data) 
	{
    	var obj = JSON.parse(data);
    	console.log(obj);
    	if(obj.code == 0)
    	{
			...
			$("#insurePrice").val(Number(obj.insurePrice).toFixed(2)+" 元");
			
			//disprice
			$("#disprice").html(Number(obj.insurePrice).toFixed(2));
			
//    		$("#mainPrice").val("1");
//    		$("#insurePrice").val("1");
			$("#prem").val(Number(obj.mainPrice).toFixed(2));
			$("#productCode").val(obj.productCode);
			$("#aliascode").val(obj.aliascode);
			$("#productName").val(obj.productName);
			if($("#productNo").val()=="060001_lnrywx"){
				getAmntDesc();
			}
    	}
    	else
