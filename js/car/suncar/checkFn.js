...
function checkEasy(idElm) {
    if (checkEmpty(idElm.val())) {
        idElm.parent().siblings(".errorMsg").css("display", "inline-block").text("请输入相应数值！");
        return false;
    }
    else {
        idElm.parent().siblings(".errorMsg").hide().text("");
...
function checkGhDate(idElm,time) {
    if (idElm.val()!='0') {
        if (checkEmpty(time.val())) {
            time.parent().siblings(".errorMsg").css("display", "inline-block").text("请输入过户日期！");
            return false;
        }else{
            time.parent().siblings(".errorMsg").hide().text("");
            return true;
        }
    }
    else {
...
/**
 * 年收入校验及错误信息提示函数
 * @param incomeCodeElm 该参数为Dom对象
 * @returns {boolean}
 */
function checkIncomeCode(incomeCodeElm) {
    if (checkEmpty(incomeCodeElm.val())) {
        incomeCodeElm.parent().siblings(".errorMsg").css("display", "inline-block").text("请输入年收入金额！");
        return false;
    } else {
        incomeCodeElm.parent().siblings(".errorMsg").hide().text("");
        return true;
    }
}
 ...
 * @returns {string}
 */
function getPersonOutNowDay(chDay){
    var aDate = new Date();
    var nowYear = aDate.getFullYear();
    var birthYear = chDay.split("-")[0];
    var result = birthYear - nowYear;
    if(result > 20){
        return false;
    }else if(result < 0){
        return false;
 ...
 * @returns
 */
function getDayByBirthDay(birth){
    start=birth.replace(/-/g,"/");
    var startdate=new Date(start);
    var myDate = new Date();
    var time=myDate.getTime()-startdate.getTime();
