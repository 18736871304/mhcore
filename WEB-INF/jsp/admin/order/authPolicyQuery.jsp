	...
	<%@ include file="/WEB-INF/common/include.jsp" %>
		...
		<head>
			...
			<script>
				...
				function dis_insuranceidcard(val, row, index) {
					if (val && val != '') {
						const maskedId = val.slice(0, 6) + '***********' + val.slice(-4);
						return maskedId
					}

				}
				function dis_insurancemobile(val, row, index) {
					if (val && val != '') {
						const maskedPhone = val.slice(0, 3) + '****' + val.slice(7);
						return maskedPhone
					}
				}
			...
			<div style="margin-left:0%">
				<table class="common">
					<tr>
						...
						<td class="report_common">
							<select class="easyui-combobox" style="width:90%" panelHeight="auto" name="qauthstate"
								id="qauthstate">
							</select>
						</td>
				...
				<table id="orderPolicyList" class="easyui-datagrid" title="认证信息" style="width:auto;height:auto"
					...
					<thead>
						<tr>
							...
							<th data-options="field:'idtypename',width:70">证件类型</th>
							<th data-options="field:'insuranceidcard',width:160,formatter:dis_insuranceidcard">证件编码</th>
							<th data-options="field:'insurancemobile',width:90,formatter:dis_insurancemobile">手机号</th>
