...
<head>
	...
	<script>
		...
		window.onload = function () {
			...
			if (pushchannel != '0001') {
				$(".activitystate").hide()
				$(".shanxi_activitystate").hide()
			}
			
			if($('#iamsoprorgan').val()=='100100100'){
				$(".shanxi_activitystate").hide()
			}else if($('#iamsoprorgan').val()=='100200100'){
				$(".activitystate").hide()
			}
		...
		function shanxi_checkauto() {
			var shanxi_checkauto = document.getElementById("shanxi_checkauto").checked
			var shanxi_checkhead = document.getElementById("shanxi_checkhead").checked
			if (shanxi_checkauto == true) {
				$("#shanxi_checkhead").prop('checked', false);
			}
			var parm = {
				activitystate: 'Y'
			}
			sendRequest("/activity/updateShanXiActivityState.do", parm, function (dataPath) {
				console.log('使用自动分配' + dataPath)
				if (JSON.parse(dataPath).code == '0') {
					$.messager.alert('操作提示', '使用自动分配成功！', 'info');
				}
		...
		function shanxi_checkhead() {
			var shanxi_checkauto = document.getElementById("shanxi_checkauto").checked
			var shanxi_checkhead = document.getElementById("shanxi_checkhead").checked
			if (shanxi_checkhead == true) {
				$("#shanxi_checkauto").prop('checked', false);
			}
			var parm = {
				activitystate: 'N'
			}
		...
		function getActivityState() {
			sendRequest("./activity/getActivityState.do", '', function (dataPath) {
				...
				if (JSON.parse(dataPath).shanxi_activitystate == "N") {
					$("#shanxi_checkauto").prop('checked', false);
					$("#shanxi_checkhead").prop('checked', true);
				}
				if (JSON.parse(dataPath).shanxi_activitystate == "Y") {
					$("#shanxi_checkhead").prop('checked', false);
					$("#shanxi_checkauto").prop('checked', true);
				}
			})
		}
...
<body>
	...
	<table class="common">
		...
		<tr class="shanxi_activitystate">
			<td class="reprot_title" style="width: 10%;">
				山西使用自动分配
			</td>
			<td class="report_common">
				<input type="checkbox" id='shanxi_checkauto' onchange="shanxi_checkauto()" style="zoom: 160%;margin: 0 4px;">
			</td>
			...
			<td class="report_common">
				<input type="checkbox" id='shanxi_checkhead' onchange="shanxi_checkhead()" style="zoom: 160%;margin: 0 4px;">
			</td>
		</tr>
		<tr class="zhubo">
